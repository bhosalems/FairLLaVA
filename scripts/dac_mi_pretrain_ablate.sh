...
# Used for the MI classifier pretraining for the ablation experiment. we train the classifier and then freeze it and only use MI in the debiasing without updating the classifier.

################## VICUNA ##################
PROMPT_VERSION=v1

PROJECTOR="/home/csgrad/mbhosale/phd/MrFair/LLaVA-Rad/checkpoints/llavarad-merged/mm_projector.bin"
model_name_or_path="/home/csgrad/mbhosale/phd/MrFair/LLaVA-Rad/checkpoints/llavarad_merged_new/" #lmsys/vicuna-7b-v1.5 ===> For fairness finetuning we do not have to start at Vicuna weights. We can rather use llava-rad directly.
...
################## Data ##################
data_path="/a2il/data/mbhosale/MrFair/physionet.org/mimc-cxr-jpeg/gpt4-reports/balanced_train_age_50k.json"
...
WANDB_PROJECT="llava" WANDB_RUN_ID="llava-ft-$(date +%Y%m%d%H%M%S)" WANDB_RUN_GROUP=fine-tune \
    ...
    --fairness_config /home/csgrad/mbhosale/phd/MrFair/LLaVA-Rad/configs/dac_mi_pretrain_mimic_cxr.json \
    --seed 42